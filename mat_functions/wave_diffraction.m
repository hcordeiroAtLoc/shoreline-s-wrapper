...
%   --------------------------------------------------------------------
    ...
    WAVE.diff2  = zeros(STRUC.nhard,size(COAST.xq,2));
    ...
    if STRUC.diffraction==1 && ~isempty(STRUC.xhard)
        ...
        if strcmpi(approach,'old')
            % original method
            omega1=repmat(dirtip(1,:)',1,nq)-phitip1;     % matrix [nhard by nq]
        ...
        if STRUC.transmission==1 %if wave propagation can take place over a submerged breakwater, then calculate amount of wave energy transmission
           for jj=1:nhard               
               ...
               [kd3obw]=wave_transmission(xhard,yhard,xq,yq,x,y,RUNUP.swl,Htip(3,jj),TPtip(3,jj),STRUC.transmbwdepth(jj),STRUC.transmcrestheight(jj),STRUC.transmcrestwidth(jj),STRUC.transmslope(jj),STRUC.transmform{jj},STRUC.transmd50(jj)); %calculate amount of wave energy transmission 
               kd3(jj,:)=kd3obw;
